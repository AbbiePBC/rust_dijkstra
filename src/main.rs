...
use crate::find_path::{get_human_readable_route, print_route, PathFinder};
...
fn main() -> Result<(), String> {
    ...
    match contents {
        ...
        Ok(file_path) => {
            ...
            while path_finder.current_route_finding <= path_finder.routes_to_find.len() {
                // todo: parallelise this &learn how to do threading in rust, for loop is slower
                let (dist, route) = path_finder.dijkstra()?;
                ...
                path_finder.current_route_finding += 1;
            }
        }
    }
